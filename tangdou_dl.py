...
def sogou_search(keyword):
    """搜狗搜索 site:tangdou.com "歌名"，尽力提取 vid。返回 [(vid, 标题)]"""
    q = urllib.parse.quote(f'site:tangdou.com "{keyword}"')
    ...
    try:
        ...
        print(f"自动搜索失败: {e}")
        return []
    # 搜狗结果链接是 /link?url=... 重定向，取回显文本里的 vid / 标题
    results = []
    for m in re.finditer(r'<h3[^>]*>\s*<a[^>]*href="(/link\?url=[^"]+)"[^>]*>([\s\S]*?)</a>', html):
        link = "https://www.sogou.com" + m.group(1)
        title = re.sub(r"<[^>]+>", "", m.group(2)).strip()
        vid = None
        try:
            with urllib.request.urlopen(urllib.request.Request(link, headers={"User-Agent": "Mozilla/5.0"}), timeout=15) as resp:
                final_url = resp.geturl()
            vid = extract_vid(final_url)
        except Exception:
            vid = None
        if vid:
...
def song_mode(keyword, outdir, want_audio, quality, log=print):
    log(f"\n== 搜索歌名: {keyword} ==")
    vids = sogou_search(keyword)
    if vids:
        log(f"自动搜索到 {len(vids)} 个糖豆视频：")
        for i, (v, t) in enumerate(vids, 1):
            log(f"  {i}. [{v}] {t}")
        try:
        ...
        if choice == "":
            ...
            chosen = [vids[int(choice) - 1]]
        for v, t in chosen:
            download_video(v, outdir, want_audio, quality, log=log)
    else:
        log("自动搜索没找到糖豆站内链接（糖豆网页搜索已下线，站内视频未被搜索引擎收录）。")
