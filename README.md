...
- ✅ **按歌名搜索（多源）**：自动搜索 **B 站**（可直接下载并合并成 mp4）+ 糖豆站内 + 全网参考；B 站结果直接加入任务下载，糖豆站内命中即自动下载，另提供 App 分享链接兜底
- ✅ **B 站全网源**：完整实现 B 站接口链路（搜索 → 视频信息 → WBI 签名 → DASH 音视频流 → ffmpeg 合并），实现"输入歌名 → 全网广场舞视频自动下载"
...
- ✅ **在线更新**：启动自动检查 GitHub Release，发现新版一键"立即更新"——后台下载（进度+可取消）→ 校验 → 自动替换并重启（打包版）；源码运行版保存到下载目录
